(function() {
	...
	Parabola.prototype = {
		...
		addOnceStep:function(){
			...
			var x, y;
			if(this.begin2){
				if (this.begin2 > this.end) {
					
				}else{
					
					//x 每一步的X轴的位置
					x = this.driftX * ((this.begin2 - this.begin) / this.duration);
					//每一步的Y轴的位置y = a*x*x + b*x + c; c==0;
					y = this.curvature * x * x + this.b * x;
					this.domove(x, y);
					console.log("this.driftX add",x,y)
					if (typeof ops.stepCallback === 'function') {
						ops.stepCallback.call(this, x, y);
					}
				}
				this.begin2 = this.begin2 + 150;
			...
			console.log(this.begin2,"this.begin2")
