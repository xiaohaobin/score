...
(function(root, factory) {
    ...
    function getQueryParams(queryString) {
        var i, l, param, tmp, paramsObj = {}, params = String(queryString).split(/&/);
        for (i = 0,
    ...
    function processJsonpRequest(requestSettings, mockHandler, origSettings) {
        ...
        if (mockHandler.response && $.isFunction(mockHandler.response)) {
            ...
            $.globalEval('(' + ((typeof mockHandler.responseText === 'string') ? ('"' + mockHandler.responseText + '"') : mockHandler.responseText) + ')');
        }
        completeJsonpCall(requestSettings, mockHandler, callbackContext, newMock);
        return newMock;
    }
    ...
    function clearByUrl(url) {
        var i, len, handler, results = [], match = url instanceof RegExp ? function(testUrl) {
        ...
        ;
        for (i = 0,
        len = mockHandlers.length; i < len; i++) {
